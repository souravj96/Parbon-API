...
import { tokenService, emailService, smsService } from '../../src/services/index.js';
import ApiError from '../../src/utils/ApiError.js';
import setupTestDB from '../utils/setupTestDB.js';
import { User, Token, OTP } from '../../src/models/index.js';
...
describe('Phone Authentication routes', () => {
  describe('POST /v1/auth/send-mobile-otp', () => {
    ...
    beforeEach(() => {
      // Mock SMS service to avoid actual SMS sending in tests
      jest.spyOn(smsService, 'sendPhoneVerificationOTP').mockResolvedValue({
        sid: 'SMmocked123456789',
    ...
    test('should return 200 and send OTP for valid phone number', async () => {
      ...
      expect(smsService.sendPhoneVerificationOTP).toHaveBeenCalledWith(validPhoneNumber);
    });
    ...
    test('should handle SMS service errors', async () => {
      smsService.sendPhoneVerificationOTP.mockRejectedValue(new Error('SMS service unavailable'));
  ...
  describe('POST /v1/auth/verify-mobile-otp', () => {
    ...
    beforeEach(async () => {
      ...
      // Mock SMS service
      jest.spyOn(smsService, 'verifyOTP').mockImplementation(async (phone, otp, purpose) => {
        if (phone === validPhoneNumber && otp === validOtp) {
    ...
    test('should return 200 and create new user with JWT tokens on successful OTP verification', async () => {
      const res = await request(app)
      ...
      const user = await User.findOne({ phoneNumber: validPhoneNumber });
      expect(user).toBeDefined();
      expect(user.isPhoneVerified).toBe(true);
    });

    test('should return 200 and login existing user with JWT tokens', async () => {
    ...
    test('should return 400 error for invalid OTP', async () => {
      const res = await request(app)
        .post('/v1/auth/verify-mobile-otp')
        .send({
          phone: validPhoneNumber,
  ...
  describe('POST /v1/auth/phone-login', () => {
    ...
    test('should return 200 and send login OTP for verified user', async () => {
      const res = await request(app)
      ...
      expect(smsService.sendPhoneVerificationOTP).toHaveBeenCalledWith(validPhoneNumber, 'login');
    });
    ...
    test('should return 400 error for unverified phone number', async () => {
      // Update user to unverified
      await User.findByIdAndUpdate(existingUser.id, { isPhoneVerified: false });
